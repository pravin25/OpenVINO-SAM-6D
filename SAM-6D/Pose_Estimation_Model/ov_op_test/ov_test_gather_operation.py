...
torch_model_path = "./test_model/gather_operation.pth"
onnx_model_path = "./test_model/gather_operation.onnx"
ov_model_path = "./test_model/gather_operation.xml"
...
class GatherOperation(torch.autograd.Function):
    ...
    def forward(ctx, features, idx):
        gather_points =  _ext.gather_points(features, idx) 
        ctx.save_for_backward(gather_points)
        return gather_points

    @staticmethod
    def symbolic(g: torch.Graph, feature: torch.Tensor , idx: torch.Tensor) :
        return g.op("GatherOperation", feature, idx, outputs=1)

class MyModel(nn.Module):
    def __init__(self):
        super(MyModel, self).__init__()
        self.gather_operation = GatherOperation.apply

    def forward(self, feature, idx):
        return self.gather_operation(feature, idx) # (B, X, Y)


def get_input_data():
    dummy_features = torch.randn(DUMMY_B, DUMMY_C, DUMMY_N, dtype=torch.float32)
    dummy_idx = torch.randint(0, DUMMY_N, (DUMMY_B, DUMMY_NPOINT), dtype=torch.int32)
    dummy_idx = dummy_idx.to(torch.int32)
    
    onnx_input = (dummy_features, dummy_idx)
    onnx_input_name = ["features", "idx"]

    ov_input = {"features":dummy_features, "idx":dummy_idx}
    ov_input_name = {"features":[-1, -1, -1], "idx":[-1, -1]}
