...
//! [op:ctor]
FurthestPointSampling::FurthestPointSampling(const ov::Output<ov::Node>& xyz, int npoint_i) 
: Op({xyz}), m_npoint(npoint_i) {
...
void FurthestPointSampling::validate_and_infer_types() {
    ...
    */
    ...
    ov::PartialShape output_shape = {xyz_shape[0], m_npoint};
...
std::shared_ptr<ov::Node> FurthestPointSampling::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    ...
    return std::make_shared<FurthestPointSampling>(new_args.at(0));
}
//! [op:copy]

//! [op:visit_attributes]
bool FurthestPointSampling::visit_attributes(ov::AttributeVisitor& visitor) {
    visitor.on_attribute("npoint", m_npoint);
    return true;
}
//! [op:visit_attributes]
