...
void CustomSVDu::validate_and_infer_types() {
    ...
    // if (rank < 2) {
    //     throw std::runtime_error("CustomSVDu input must have at least 2 dimensions (batch..., M, N)");
    // }
...
bool CustomSVDu::evaluate(ov::TensorVector& outputs, const ov::TensorVector& inputs) const {
    ...
    if (debug) {
        ...
        if (fp) {
            ...
            std::cerr << "[CustomSVDu Debug] Failed to open output/ov_svd.txt for writing!" << std::endl;
        }
    }
